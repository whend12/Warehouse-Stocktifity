...
const Sidebar = ({children}) => {
    ...
    return (
          ...
          <div className="flex relative">
              <div className={`${isOpen ? "w-[250px]" : "w-16"} fixed min-h-screen duration-500 bg-[#6B728E] border-r-2`}>
                  ...
                  { menuItem.map((item, index) => (
                      <NavLink to={item.path} key={index} className="link flex items-center text-white pt-2.5 pb-2.5 pl-3.5 pr-3.5 gap-4 duration-500 hover:bg-[#EEEEEE] hover:text-[#000] active:bg-blue-500">
                          <div key={item.icon} className={`${!isOpen && "ml-2"} icon text-lg`}>{item.icon}</div>
                          <div key={item.name} className={`${!isOpen && "hidden"} link-text text-sm`}>{item.name}</div>
                      </NavLink>
                  ))
              }
            ...
            <main className={`${isOpen ? "ml-[250px]" : "ml-[50px]"} w-screen duration-500`}>{children}</main>
          </div>
          </>        
    )
}
